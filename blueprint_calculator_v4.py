...
# -*- coding: utf-8 -*-
"""
抽图纸概率计算器 v4.0 - 基于Beta-Binomial分布模型
合并118个样本数据重新拟合 (成功率约12.50%)
...
# ==================== 概率模型参数 (v4.0 - 118样本合并) ====================
# 合并新旧样本后拟合的Beta分布参数
ALPHA = 9.7881          # Beta分布α参数
BETA_PARAM = 68.5089    # Beta分布β参数
MEAN_SUCCESS_RATE = ALPHA / (ALPHA + BETA_PARAM)  # 约0.1250 (12.50%)
...
def option1_pt_for_target_blueprints():
    ...
    plt.tight_layout()
    filename = f'v4_option1_target{target}.png'
...
def option2_additional_pt_needed():
    ...
    plt.tight_layout()
    filename = f'v4_option2_{current_blueprints}to{target_blueprints}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"\n图表已保存至: {filename}")
    plt.close()
...
def option3_probability_of_current_state():
    ...
    plt.tight_layout()
    filename = f'v4_option3_{current_blueprints}bp_{current_pt}pt.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"\n图表已保存至: {filename}")
    plt.close()
