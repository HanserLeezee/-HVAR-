...
# -*- coding: utf-8 -*-
"""
抽图纸概率计算器 v5.0 - 基于Beta-Binomial分布模型
合并181个样本数据重新拟合 (成功率约12.54%)
...
# ==================== 概率模型参数 (v5.0 - 181样本合并) ====================
# 合并三批样本后拟合的Beta分布参数
ALPHA = 10.3198         # Beta分布α参数
BETA_PARAM = 71.9954    # Beta分布β参数
MEAN_SUCCESS_RATE = ALPHA / (ALPHA + BETA_PARAM)  # 约0.1254 (12.54%)

# 基础游戏参数
BASE_SCORE = 12900     # 基础积分
...
def print_header():
    """打印程序标题"""
    print("=" * 60)
    print("      抽图纸概率计算器 v5.0 - 181样本合并模型")
    print("=" * 60)
    print(f"\n模型参数:")
    print(f"  - Beta分布: α = {ALPHA:.2f}, β = {BETA_PARAM:.2f}")
    print(f"  - 单次成功率: {MEAN_SUCCESS_RATE:.4f} ({MEAN_SUCCESS_RATE*100:.2f}%)")
...
def option1_pt_for_target_blueprints():
    ...
    plt.tight_layout()
    filename = f'v5_option1_target{target}.png'
...
def option2_additional_pt_needed():
    ...
    plt.tight_layout()
    filename = f'v5_option2_{current_blueprints}to{target_blueprints}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"\n图表已保存至: {filename}")
    plt.close()
...
def option3_probability_of_current_state():
    ...
    plt.tight_layout()
    filename = f'v5_option3_{current_blueprints}bp_{current_pt}pt.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"\n图表已保存至: {filename}")
    plt.close()
